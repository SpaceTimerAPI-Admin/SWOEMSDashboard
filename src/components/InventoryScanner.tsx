/**
 * InventoryScanner — fullscreen inventory lookup modal for the office dashboard.
 * Supports barcode scan (camera via BarcodeInput), manual serial entry, and text search.
...
export default function InventoryScanner({ onClose }: Props) {
  ...
  useEffect(() => {
    ...
    searchTimer.current = setTimeout(async () => {
      setSearching(true);
      try { const res = await searchItems(searchInput); setSearchResults(res.items || []); } finally { setSearching(false); }
    }, 300);
  }, [searchInput]);

  async function doLookup(serial: string) {
    if (!serial.trim()) return;
    setLooking(true);
    try {
      const res = await lookupSerial(serial);
      if (res.found && res.item) { setItem(res.item); setMode("result"); }
      else { setScannedSerial(serial); setMode("new"); }
    } finally { setLooking(false); }
  }

  function reset() {
    setMode("scan"); setSerialInput(""); setSearchInput("");
    setItem(null); setScannedSerial("");
    setNewName(""); setNewModel(""); setNewManuf(""); setSaveError(null);
  }
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      const res = await createItem({ serial_number: scannedSerial || undefined, name: newName.trim(), manufacturer: newManuf.trim() || undefined, model: newModel.trim() || undefined, category_name: newCategory, location: newLocation.trim() || "Shop" });
      if (!res.ok) throw new Error(res.error || "Failed to create");
      setItem(res.item); setMode("result");
    } catch (err: any) { setSaveError(err?.message || "Failed"); } finally { setSaving(false); }
  }
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 300, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}
      ...
      <div style={{ width: "100%", maxWidth: 500, background: "#0f1117", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 20, overflow: "hidden", boxShadow: "0 24px 80px rgba(0,0,0,0.7)", display: "flex", flexDirection: "column", maxHeight: "90vh" }}>
          ...
          {mode !== "new" && (
            <div style={{ display: "flex", gap: 6, marginTop: 10 }}>
              {[["scan","🔍 Scan / Serial"],["search","🔎 Search"]].map(([m,label]) => (
                <button key={m} onClick={() => { setMode(m as any); setItem(null); }}
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "14px 18px" }}>
          ...
          {mode === "scan" && (
            <div>
              <div style={{ fontSize: 13, color: "#9ca3af", marginBottom: 12 }}>
                Tap <strong style={{ color: "#c7d2fe" }}>📷 Scan</strong> to use your camera, or type the serial number manually
              </div>
              <BarcodeInput
                value={serialInput}
                onChange={setSerialInput}
                placeholder="Type serial or asset tag…"
                autoFocus
                onEnter={() => doLookup(serialInput)}
              />
              {serialInput.trim() && (
                <button onClick={() => doLookup(serialInput)} disabled={looking} className="btn primary small" style={{ marginTop: 10, width: "100%" }}>
                  {looking ? <><span className="spinner" style={{ marginRight: 6 }} />Looking up…</> : "Look Up"}
                </button>
              )}
            </div>
          )}
          ...
          {mode === "search" && (
            <div>
              <div style={{ fontSize: 13, color: "#9ca3af", marginBottom: 10 }}>Search by name, manufacturer, model, or location</div>
              <input ref={searchRef} className="input" value={searchInput} onChange={e => setSearchInput(e.target.value)} placeholder="e.g. Elation SixPar, Main Gate, ETC D40…" />
              <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 6 }}>
                ...
                {searchResults.map(i => {
                  ...
                  return (
                    <div key={i.id} onClick={() => { setItem(i); setMode("result"); setSearchInput(""); }}
          ...
          {mode === "result" && item && sm && (
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 700, padding: "3px 11px", borderRadius: 99, flexShrink: 0, background: sm.bg, color: sm.color, border: `1px solid ${sm.color}44` }}>{sm.label}</span>
              </div>
              ...
              {item.notes && <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 8, padding: "9px 12px", fontSize: 12, color: "#9ca3af", lineHeight: 1.5, marginBottom: 14 }}>{item.notes}</div>}
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                <button onClick={reset} className="btn small">🔍 Scan Another</button>
                <a href={`/inventory/${item.id}`} target="_blank" rel="noreferrer" style={{ textDecoration: "none" }} className="btn small">View Full Record ↗</a>
              </div>
            </div>
          )}
          ...
          {mode === "new" && (
            <form onSubmit={handleCreate}>
              <div style={{ background: "rgba(251,191,36,0.08)", border: "1px solid rgba(251,191,36,0.2)", borderRadius: 8, padding: "10px 12px", fontSize: 13, color: "#fcd34d", marginBottom: 14 }}>
                {scannedSerial ? <>Serial <span style={{ fontFamily: "monospace" }}>{scannedSerial}</span> not found — add it:</> : "Add new item:"}
              </div>
              {scannedSerial && <div style={{ marginBottom: 10, fontFamily: "monospace", fontSize: 13, color: "#c7d2fe" }}>{scannedSerial}</div>}
              ...
              <div style={{ display: "flex", gap: 10, marginBottom: 10 }}>
                <label style={{ flex: 1 }}><div className="field-label">Manufacturer</div><input className="input" value={newManuf} onChange={e => setNewManuf(e.target.value)} placeholder="e.g. Elation" /></label>
                <label style={{ flex: 1 }}><div className="field-label">Model</div><input className="input" value={newModel} onChange={e => setNewModel(e.target.value)} placeholder="e.g. SixPar 1000" /></label>
              </div>
              <div style={{ marginBottom: 10 }}>
                <div className="field-label">Category</div>
                <div className="tag-row">{["Lighting","Sound","Video","Rides","Other"].map(c => <button key={c} type="button" className={`tag-btn${newCategory===c?" active":""}`} onClick={()=>setNewCategory(c)}>{c}</button>)}</div>
              </div>
              <label style={{ display: "block", marginBottom: 14 }}><div className="field-label">Location</div><input className="input" value={newLocation} onChange={e => setNewLocation(e.target.value)} placeholder="e.g. Shop" /></label>
              {saveError && <div style={{ fontSize: 12, color: "#FFB0B0", marginBottom: 10 }}>⚠ {saveError}</div>}
              <div style={{ display: "flex", gap: 8 }}>
                <button type="submit" disabled={saving} className="btn primary small">{saving ? <><span className="spinner" style={{ marginRight: 6 }} />Adding…</> : "Add to Inventory"}</button>
