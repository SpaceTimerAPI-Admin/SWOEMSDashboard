...
export default function ShowTechHome() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res: any = await listTickets({ includeClosed: false });
        const all = res?.ok ? (res?.data?.tickets || res?.tickets || []) : [];
        // Show Tech sees tickets created by show_tech users
        // We rely on created_by_role if available, otherwise filter by assigned to show_tech pool
        // For now show tickets assigned to this user or to "show_tech" pool
        const mine = all.filter((t: any) =>
          t.assigned_to === profile?.id ||
          t.created_by === profile?.id ||
          t.created_by_role === "show_tech"
        );
        setMyTickets(mine.filter((t: any) => !isClosed(t)));
      } catch {}
      setLoading(false);
    }
    void load();
  ...
  return (
    <div className="page fade-up">
      ...
      <div style={{ marginBottom: 20 }}>
        <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", color: "var(--muted2)", marginBottom: 8 }}>
          My Open Tickets ({loading ? "…" : myTickets.length})
        </div>
        {loading ? (
          <div className="muted" style={{ fontSize: 13 }}>Loading…</div>
        ) : myTickets.length === 0 ? (
          <div className="card" style={{ padding: "20px", textAlign: "center" }}>
            <div style={{ fontSize: 22, marginBottom: 6 }}>✓</div>
            <div style={{ fontSize: 13, color: "var(--muted)" }}>No open tickets assigned to you.</div>
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {myTickets.slice(0, 5).map((t: any) => (
              <Link key={t.id} to={`/tickets/${t.id}`} className="card"
                ...
                <div style={{ fontSize: 11, color: "var(--muted2)" }}>📍 {t.location}{t.tag ? ` · ${t.tag}` : ""}</div>
              </Link>
            ))}
            {myTickets.length > 5 && (
              <Link to="/tickets" style={{ fontSize: 13, color: "#B0B8FF", textAlign: "center", padding: "8px" }}>
