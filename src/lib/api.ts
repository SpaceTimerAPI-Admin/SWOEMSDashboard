  ...
  try {
    ...
    if (typeof payload === "object" && payload && "ok" in payload) {
      if ((payload as any).ok === false)
        return {
          ok: false,
          error: (payload as any).error ?? "Request failed",
          status: res.status,
        };
...
export async function listTickets(opts?: {
  ...
  return apiFetch<{ tickets: Ticket[] }>("/api/tickets-list", {
    method: "POST",
    body: opts ?? {},
  });
}
...
export async function getTicket(
  id: string,
): Promise<ApiResult<{ ticket: Ticket; comments?: any[] }>> {
  return apiFetch<{ ticket: Ticket; comments?: any[] }>("/api/tickets-get", {
    method: "POST",
    body: { id },
  });
}
...
export async function addTicketComment(input: {
  ...
  return apiFetch<{}>("/api/tickets-comment", {
    method: "POST",
    body: { id, comment: input.comment, photo_keys: input.photo_keys ?? [] },
  });
}
...
export async function confirmTicketPhoto(input: {
  ticket_id: string;
  storage_key?: string;
  storage_path?: string;
}): Promise<ApiResult<{}>> {
  return apiFetch<{}>("/api/tickets-photo-confirm", {
    ...
    body: {
      ticket_id: input.ticket_id,
      storage_key: input.storage_key ?? input.storage_path,
    },
  });
}
...
export async function listProjects(opts?: {
  ...
  return apiFetch<{ projects: Project[] }>("/api/projects-list", {
    method: "POST",
    body: opts ?? {},
  });
}
...
export async function getProject(
  id: string,
): Promise<ApiResult<{ project: Project; comments?: any[] }>> {
  return apiFetch<{ project: Project; comments?: any[] }>("/api/projects-get", {
    method: "POST",
    body: { id },
  });
}
...
export async function addProjectComment(input: {
  ...
  return apiFetch<{}>("/api/projects-comment", {
    method: "POST",
    body: { id, comment: input.comment, photo_keys: input.photo_keys ?? [] },
  });
}
...
export async function confirmProjectPhoto(input: {
  project_id: string;
  storage_key?: string;
  storage_path?: string;
}): Promise<ApiResult<{}>> {
...
export async function sendEod(payload: {
  to?: string;
  subject?: string;
  notes?: string;
  ...
  return apiFetch<{}>("/api/send-eod", {
    method: "POST",
    body: { to: payload.to, subject: payload.subject, notes },
  });
}
