...
export const handler: Handler = async (event) => {
  try {
    ...
    const sender_type    = payload?.sender_type ? String(payload.sender_type) : null;
    ...
    // Ignore bot messages (including our own) — prevents any possibility of loops
    if (sender_type === "bot") return json({ ok: true, ignored: true });

    // Also ignore by bot name as belt-and-suspenders
