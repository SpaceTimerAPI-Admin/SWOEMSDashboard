...
export const handler: Handler = async (event) => {
  try {
    ...
    return json({
      ok: true,
      project: { ...project, created_by_name: (project as any).employees?.name || "Unknown", assigned_to_name: (project as any).assignee?.name || null, assigned_to_show_tech: (project as any).assigned_to_show_tech || false },
