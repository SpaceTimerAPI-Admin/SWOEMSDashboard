      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:640px;margin:0 auto">
        ...
        <tr><td style="background:#FFFFFF;border:1px solid #E8E8E8;border-top:0;border-radius:0 0 14px 14px;padding:24px 28px">
          ...
          <div style="margin-bottom:24px">
            <div style="font-size:13px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:#888;margin-bottom:10px">📓 Shift Log (${shiftLogEntries.length} entr${shiftLogEntries.length !== 1 ? "ies" : "y"})</div>
            ${shiftLogEntries.map((e: any) => `
              <div style="padding:10px 14px;background:#F8F7FF;border:1px solid #E8E4FF;border-radius:8px;margin-bottom:8px">
                ...
                <div style="font-size:13px;color:#333;line-height:1.5">${escapeHtml(e.note).replace(/\n/g, "<br>")}</div>
              </div>
            `).join("")}
          </div>
          ` : ""}
          ...
          ${(shiftLogEntries.length > 0 || handoffNotes.trim()) ? `<hr style="border:none;border-top:1px solid #F0F0F0;margin:0 0 24px">` : ""}
...
export const handler: Handler = async (event) => {
  try {
    ...
    await supabase.from("end_of_day_reports").insert({
      ...
      notes: "",
      handoff_notes,
      snapshot_json: { tickets: allTickets, projects: allProjects, shift_log: shiftLogEntries },
      emailed_to: to,
      email_status,
      email_error,
    });
    ...
    return json({ ok: true, emailed_to: to, ticket_count: allTickets.length, project_count: allProjects.length, shift_log_count: shiftLogEntries.length });
  } catch (e: any) {
